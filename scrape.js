...
function writingToEndPoint(startFrom) {
	...
	asyncLoop(start, function(loop) {
		...
		request({url : links[l+startFrom],jar:j}, function(error, response, body) {
					...
					for (var z = 0; z < patternArr.length; z++) {
						if (get.method == "asos") {
							if (get.finalSearch[m] == "#ctl00_ContentMainPage_divmor>ul>li>a"){
								...
								results.push(temp[0].children[0].data);
								}
								else{
								results.push("null");
								}
	...
	//										console.log(temp2[i].children[4].children[0].data);											
		//									string+=temp2[i].children[4].children[0].data;
					...
					if(get.method == "asos"){
						if(results.length!=6){
							console.log(results);
							console.log("Scraper didn't retrieve results. X_X.");
							process.exit(1);
						}	
					}
					...
					for (var n = 0; n < results.length; n++) {
						...
						if(get.method == "asos"){
							...
							resultString+=connection.escape(results[n]);
							if(n == results.length-1){
								resultString+=","+connection.escape(links[l+startFrom]);
							}
						}
						if (get.method == "generic") {
